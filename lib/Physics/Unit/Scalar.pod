...
returns a Physics::Unit::Scalar object.  If $arg is an object reference,
...
This returns a new object which is the product of $self and the
argument.  Neither the original object nor the argument is changed.
FIXME:  These should all work the same way.  Either all change the
value of the existing object, or all return a new object.
...
FIXME:  Why is the Unit method "divide", but the Scalar method
is "div"?

=head1 EXPORTS

FIXME:  Need this section.
